...
/*
    ...
    lpKhh       [in]        客户号
    lpJymm      [in]        交易密码
    lpZqdm      [in]        证券代码（为空就查询所有）
    lpQsrq      [in]        起始日期
    lpJsrq      [in]        结束日期
    nRowCount   [in]        查询行数（<=200）
    lpSyrq      [in]        索引日期
    lpSyYybdm   [in]        索引营业部代码
    lpBrowindex [in]        索引流水号
    lpWtfs      [in]        委托方式
    sFXFS       [in]        发行方式
    arXGZQ      [out]       结果数据
    return                  成功返回数量，失败返回<0
*/
ITPDK_API int64 SECITPDK_QueryXGZQ(const char *lpYybdm, const char *lpScdm, const char *lpKhh, const char *lpJymm, const char *lpZqdm, const char *lpQsrq, const char *lpJsrq, int nRowCount, const char *lpSyrq, const char *lpSyYybdm, const char *lpBrowindex, const char *lpWtfs, const char *sFXFS, vector<ITPDK_XGZQ>& arXGZQ);
...
/*
    ...
    return                  成功返回数量，失败返回<0
*/
ITPDK_API int64 SECITPDK_QueryPHCX(const char *lpYybdm, const char *lpScdm, const char *lpKhh, const char *lpJymm, const char *lpGddm, const char *lpPhdm, const char *lpQsrq, const char *lpJsrq, int nRowCount, const char *lpBrowindex, const char *lpWtfs, const char *lpZjzh, vector<ITPDK_PHCX>& arPHCX);
...
/*
    ...
    nRowCount   [in]        查询行数（<=200）
    lpBrowindex [in]        索引流水号
    nExFlag     [in]        是否取辅账户流水（0:否 1:是，默认为 0）
    arYZYWLS    [out]       结果数据
    return                  成功返回数量，失败返回<0
*/
ITPDK_API int64 SECITPDK_QueryYZYWLS(const char *lpKhh, const char *lpZjzh, const char *lpJymm, const char *lpScdm, const char *lpYyb, const char *lpBz, const char *lpSqh, const char *lpYhdm, const char *lpYhzh, int nRowCount, const char *lpBrowindex, int nExFlag, vector<ITPDK_YZYWLS>& arYZYWLS);
...
/*
    ...
    return                  成功返回数量，失败返回<0
*/
ITPDK_API int64 SECITPDK_QueryYHDM(const char *lpKhh, const char *lpJymm, const char *lpScdm, const char *lpYyb, int nYHLB, int nExFlag, const char *lpWtfs, const char *lpBz, const char *lpYhdm, vector<ITPDK_YHDM>& arYHDM);
...
/*
    ...
    return                  成功返回>0，失败返回<0
*/
ITPDK_API int64 SECITPDK_QueryYHYE(const char *lpKhh, const char *lpJymm, const char *lpZjzh, const char *lpZjmm, const char *lpBz, const char *lpYhzh, const char *lpYhdm, const char *lpYhmm, const char* lpWbzhmm, vector<ITPDK_YHYE>& arYHYE);

/*
    银证转账业务

    lpKhh       [in]        客户号
    nYwlb       [in]        业务类别（YWLB_YHTOZQ银行转证券、YWLB_ZQTOYH证券转银行）
    lpZjzh      [in]        资金账号
    lpJymm      [in]        交易密码
    lpZjmm      [in]        资金密码
    lpBz        [in]        币种
    ...
    return                  成功返回>0，失败返回<0
*/
ITPDK_API int64 SECITPDK_BankBusiness(const char *lpKhh,int nYwlb, const char *lpZjzh, const char *lpJymm, const char *lpZjmm, const char *lpBz, const char *lpYhzh, double lfZzje, const char *lpYhdm, const char *lpYhmm);
